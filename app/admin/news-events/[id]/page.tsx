...
export default function EditNewsEventPage({ params }: Props) {
  ...
  useEffect(() => {
    function parseSingleDate(dateStr: string) {
      if (!dateStr) return "";
    ...
    async function loadData() {
      ...
      try {
        ...
        if (res.ok && data.newsEvent) {
          const formattedDate = parseSingleDate(data.newsEvent.date) || data.newsEvent.date;
          setFormData({ ...data.newsEvent, date: formattedDate });
          if (data.newsEvent.category) {
            setCategoriesList((prev) => Array.from(new Set([...prev, data.newsEvent.category])));
          }
        } else {
      ...
      <form onSubmit={handleSubmit} className="space-y-8 rounded-xs border border-palette-sand/70 bg-bg p-8">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          ...
          */}
          ...
          <div className="space-y-1.5">
            <label className="block font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-palette-amber">
              Event Date / Publication Date ({formData.date}) *
            </label>
            <input
              type="date"
              required
